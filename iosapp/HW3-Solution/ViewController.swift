...
class ViewController: UIViewController, SettingsViewControllerDelegate {
    ...
    @IBAction func modePressed(_ sender: UIButton) {
        ...
        case .Length:
            ...
            changePlaceholderText(fromString: "Enter volume in \(fromUnits.text!)", toString: "Enter volume in \(toUnits.text!)")
        case .Volume:
            currentMode = .Length
            fromUnits.text = LengthUnit.Yards.rawValue
            toUnits.text = LengthUnit.Meters.rawValue
            changePlaceholderText(fromString: "Enter length in \(fromUnits.text!)", toString: "Enter length in \(toUnits.text!)")
        }
        calculatorHeader.text = "\(currentMode.rawValue) Conversion Calculator"
        
    }
    
    func changePlaceholderText(fromString: String, toString: String) {
        fromField.placeholder = fromString
        toField.placeholder = toString
        //Change the placeholder text color.
        guard let fromPlaceHolder = fromField.placeholder else {
            return
        }
        
        fromField.attributedPlaceholder = NSAttributedString(string: fromPlaceHolder, attributes: [NSAttributedStringKey.foregroundColor :FOREGROUND_COLOR])
        
        guard let toPlaceHolder = toField.placeholder else {
            return
        }
        
        toField.attributedPlaceholder = NSAttributedString(string: toPlaceHolder, attributes: [NSAttributedStringKey.foregroundColor :FOREGROUND_COLOR])
    }
    ...
    func settingsChanged(fromUnits: LengthUnit, toUnits: LengthUnit)
    {
        ...
        changePlaceholderText(fromString: "Enter length in \(fromUnits.rawValue)", toString: "Enter length in \(toUnits.rawValue)")
    }
    ...
    func settingsChanged(fromUnits: VolumeUnit, toUnits: VolumeUnit)
    {
        self.fromUnits.text = fromUnits.rawValue
        self.toUnits.text = toUnits.rawValue
        changePlaceholderText(fromString: "Enter volume in \(fromUnits.rawValue)", toString: "Enter volume in \(toUnits.rawValue)")
    }
}
